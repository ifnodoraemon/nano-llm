...
def get_fineweb_edu_files(config_name: str) -> list:
    """
    Queries Hugging Face mirror API to list all parquet files for the config.
    """
    url = "https://hf-mirror.com/api/datasets/HuggingFaceFW/fineweb-edu"
    r = requests.get(url, timeout=30)
    r.raise_for_status()
    data = r.json()
    all_files = [s['rfilename'] for s in data['siblings']]
    
    # Filter by config
    if config_name == "sample-100BT":
        prefix = "sample/100BT/"
    elif config_name == "sample-10BT":
        prefix = "sample/10BT/"
    elif config_name == "sample-350BT":
        prefix = "sample/350BT/"
    else:
        # Default is CC-MAIN crawls
        prefix = "data/"
        
    filtered = [f for f in all_files if f.startswith(prefix) and f.endswith(".parquet")]
    filtered.sort()
    return filtered

def get_skypile_files() -> list:
    """
    Queries Hugging Face mirror API to list all jsonl files for SkyPile.
    """
    url = "https://hf-mirror.com/api/datasets/Skywork/SkyPile-150B"
    r = requests.get(url, timeout=30)
    r.raise_for_status()
    data = r.json()
    all_files = [s['rfilename'] for s in data['siblings']]
    filtered = [f for f in all_files if f.startswith("data/") and f.endswith(".jsonl")]
    filtered.sort()
    return filtered

def worker_tokenize_task_file_sharded(
    worker_idx: int,
    num_workers: int,
    dataset_type: str, # "en" (parquet) or "zh" (jsonl)
    file_list: list,   # Sharded files assigned to this worker
    target_tokens: int,
    output_dir: str,
    prefix: str,
    val_ratio: float,
):
    # Set mirror in environment for subprocesses
    ...
    proc_logger.info(f"🚀 Worker {worker_idx}/{num_workers} starting for {dataset_type} ({prefix}). Target: {target_tokens:,} tokens. Assigned files: {len(file_list)}")
    ...
    try:
        ...
        if dataset_type == "en":
            base_url = "https://hf-mirror.com/datasets/HuggingFaceFW/fineweb-edu/resolve/main/"
        else:
            base_url = "https://hf-mirror.com/datasets/Skywork/SkyPile-150B/resolve/main/"
            
        # Add random startup jitter to prevent concurrent api rate-limiting spikes
        startup_delay = random.uniform(1.0, 10.0)
        proc_logger.info(f"Waiting {startup_delay:.1f}s jitter delay...")
        ...
        for file_path in file_list:
            ...
            proc_logger.info(f"Downloading and processing: {file_path}")
            ...
            for retry in range(max_retries):
                try:
                    ...
                    sleep_time = random.uniform(5.0, 15.0) * (retry + 1)
                    proc_logger.warning(f"Error reading file {file_path} (retry {retry+1}/{max_retries}): {e}. Retrying in {sleep_time:.1f}s...")
                    time.sleep(sleep_time)
            
            if not success:
                proc_logger.error(f"❌ Failed to process file {file_path} after {max_retries} retries. Skipping.")
                continue
                
            proc_logger.info(f"Loaded {len(texts):,} documents from {file_path}. Starting tokenization...")
            
            # Process documents in the file
            for text in texts:
                try:
                ...
                if doc_count % 50000 == 0:
                    proc_logger.info(f"Worker {worker_idx} processed {doc_count:,} docs | Total tokens written: {total_tokens_written:,}")
                    
                if total_tokens_written >= target_tokens:
                    proc_logger.info(f"🎯 Target token count reached ({total_tokens_written:,} tokens).")
                    break
...
def main():
    ...
    parser.add_argument("--skip_docs", type=int, default=0, help="Number of documents to skip per worker (deprecated in file-level sharding)")
    ...
    # --------------------------------------------------------------------------
    if target_en > 0:
        logger.info("📦 Phase 1/2: Listing and downloading English educational corpus (FineWeb-Edu)...")
        # Automatically choose appropriate config based on token count
        if target_en <= 8e9:
            config_choice = "sample-10BT"
        elif target_en <= 80e9:
            config_choice = "sample-100BT"
        elif target_en <= 300e9:
            config_choice = "sample-350BT"
        else:
            config_choice = "default"
            
        logger.info(f"Determined config choice: {config_choice}")
        all_en_files = get_fineweb_edu_files(config_choice)
        logger.info(f"Found {len(all_en_files)} parquet files for English corpus.")
        
        # Divide files among workers
        sharded_files = [[] for _ in range(args.num_workers)]
        for idx, file_path in enumerate(all_en_files):
            sharded_files[idx % args.num_workers].append(file_path)
        ...
        for i in range(args.num_workers):
            p = mp.Process(
                target=worker_tokenize_task_file_sharded,
                args=(
                    i,
                    args.num_workers,
                    "en",
    ...
    # --------------------------------------------------------------------------
    if target_zh > 0:
        logger.info("📦 Phase 2/2: Listing and downloading Chinese corpus (Skywork/SkyPile-150B)...")
        all_zh_files = get_skypile_files()
        logger.info(f"Found {len(all_zh_files)} jsonl files for Chinese corpus.")
        
        # Divide files among workers
        sharded_files = [[] for _ in range(args.num_workers)]
        for idx, file_path in enumerate(all_zh_files):
            sharded_files[idx % args.num_workers].append(file_path)
