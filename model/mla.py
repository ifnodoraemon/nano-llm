...
class MultiHeadLatentAttention(nn.Module):
    ...
    def forward(
        ...
        if cache_manager is not None and seq_id is not None:
            ...
            if not hasattr(self, 'freqs_cos') or self.freqs_cos.shape[0] < start_pos + seqlen:
                freqs_cis_k_full = precompute_freqs_cis(
                    dim=self.head_dim,
                    end=max((start_pos + seqlen) * 2, 4096),
                    scaling_factor=self.config.rope_scaling
                ).to(xq.device)
                self.freqs_cos = freqs_cis_k_full.real
                self.freqs_sin = freqs_cis_k_full.imag
                
            freqs_k = (self.freqs_cos[start_pos : start_pos + seqlen], self.freqs_sin[start_pos : start_pos + seqlen])
            ...
            if seqlen == 1:
                ...
                attn_out = kernel(q, seq_id, cache_manager, seq_len=start_pos + seqlen)
                output = attn_out.transpose(1, 2).contiguous().view(bsz, seqlen, -1)
            else:
        ...
        elif kv_cache is not None and start_pos is not None:
